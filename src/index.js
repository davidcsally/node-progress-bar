...
const progBar = (i) => {
  const dashes = i * (BAR_LENGTH * .01) % BAR_LENGTH;
  const spaces = BAR_LENGTH - dashes;
  if (i === 100) return new Array(BAR_LENGTH).join('-')
  return Array.apply(null, Array(BAR_LENGTH)).map((v, i) => {
    if (i < dashes-1) return '-';
    return ' ';
  }).join('');
};
...
const printProgress = (index, total, options = {}) => {
  ...
  process.stdout.write(`Progress: ${bufferMessage(percent)}% complete [${progBar(percent)}]                \r`);
};
